...
public class PartitionAwarePolicyQueryPlanTest extends LoadBalancingPolicyTestBase {
  ...
  public void should_use_leader_at_quorum_cl() {
    ...
    given(metadata.getDefaultPartitionMetadata()).willReturn(Optional.ofNullable(partitionMetadata));
    given(partitionMetadata.getTableSplitMetadata(anyString(), anyString())).willReturn(tableSplitMetadata);
  ...
  public void should_round_robin_at_cl_one_single_dc() {
    ...
    given(partitionMetadata.getTableSplitMetadata(anyString(), anyString())).willReturn(tableSplitMetadata);
    given(tableSplitMetadata.getHosts(anyInt())).willReturn(ImmutableList.of(node1, node2, node3));
    // When TODO CL ONE randomly shuffles the node list. need to figure out if there's a way to test properly
